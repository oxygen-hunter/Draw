...
XPainter::XPainter(QWidget *parent) :
    QMainWindow(parent)
{
...
void XPainter::paintEvent(QPaintEvent *)
{
...
void XPainter::mousePressEvent(QMouseEvent *e)
{
...
void XPainter::AddTexts()//当输入框文本改变时调用
{
    QString& last = _text.last();//拿到最后一个文本
    last = _tEdit->toPlainText();//将输入框文本作为文本
}

void XPainter::mouseMoveEvent(QMouseEvent *e)
{
...
void XPainter::mouseReleaseEvent(QMouseEvent *e)
{
    if(_lpress)
    {
        if(_drawType == 1)
...
void XPainter::SavePic()
{
    ...
    if (fileName.length() > 0)
    {
        ...
        QPainter XPainter(&pixmap);//将pixmap作为画布
        XPainter.fillRect(QRect(0, 0, size().width(), size().height()), Qt::white);//设置绘画区域、画布颜色
        this->render(&XPainter);//将窗体渲染到XPainter，再由XPainter画到画布
        pixmap.copy(QRect(0,30,size().width(),size().height()-30)).save(fileName);//不包含工具栏
    }
}

void XPainter::OpenPic()
{
    //弹出文件打开对话框
...
void XPainter::contextMenuEvent(QContextMenuEvent *)  //右键菜单事件
{
    _Rmenu->exec(cursor().pos());//在光标位置弹出菜单
}

void XPainter::keyPressEvent(QKeyEvent *e) //按键事件
{
     //Ctrl+Z撤销
